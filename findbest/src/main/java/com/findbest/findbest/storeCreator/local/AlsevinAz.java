...
public class AlsevinAz extends LocalStore {
    private final List<String> links;
    private final List<StoreResponseDto> responseDtoList = new ArrayList<>();

    private final static String SPECIAL_PRICE_ELEMENT_KEY = "p.special-price span.price";
    private final static String REGULAR_PRICE_ELEMENT_KEY = "div.price-box span.regular-price span.price";
    private final static String PRODUCT_NAME_ELEMENT_KEY = "div.product-name h1";
    private final static String PRODUCT_PICTURE_KEY = "p.product-image a";



    public AlsevinAz(List<String> links){
        super("AlsevinAz");
    ...
    private String getPriceElement(Document doc, String link){
        Element specialPriceElement = doc.selectFirst(SPECIAL_PRICE_ELEMENT_KEY);
        Element regularPriceElement = doc.selectFirst(REGULAR_PRICE_ELEMENT_KEY);

        if(regularPriceElement != null)
            return regularPriceElement.text().trim().split(" ")[1];

        if(specialPriceElement == null)
            throw new FieldNotFoundInStoreHTMLException(link, STORE_NAME, "price");

        return specialPriceElement.text().trim().split(" ")[1];
    }

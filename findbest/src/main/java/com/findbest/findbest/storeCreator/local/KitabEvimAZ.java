...
public class KitabEvimAZ extends LocalStore{
    private final List<String> links;
    private final List<StoreResponseDto> responseDtoList = new ArrayList<>();

    private final static String PRICE_ELEMENT_KEY = "p.price span";
    private final static String PRODUCT_NAME_ELEMENT_KEY = "h1.product_title";
    private final static String PRODUCT_PICTURE_KEY = ".woocommerce-product-gallery__image a";



    public KitabEvimAZ(List<String> links){
        super("KitabEvimAZ");
    ...
    private String getPriceElement(Document doc, String link){
        Elements priceElement = doc.select(PRICE_ELEMENT_KEY);
        if(priceElement == null)
            throw new FieldNotFoundInStoreHTMLException(link, STORE_NAME, "price");


        return priceElement.text().trim().split(" ")[3];
    }
